...
export default function Hero({ onExplore }: { onExplore: () => void }) {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setCurrentSlide((prev: number) => (prev + 1) % NEW_HERO_SLIDES.length);
    }, 6000);
  ...
  return (
    <section className="relative w-full h-[95vh] min-h-[640px] max-h-[1080px] overflow-hidden flex flex-col justify-between pt-24 sm:pt-28 md:pt-32 pb-6 sm:pb-8 bg-[#0F172A]">
      ...
      <div className="relative z-20 max-w-7xl mx-auto px-5 sm:px-8 md:px-14 w-full my-auto py-8 sm:py-12">
        <div className="max-w-3xl space-y-4 sm:space-y-6 flex flex-col items-start text-left">
          <motion.div
            animate={{ y: [0, -5, 0] }}
            transition={{ duration: 3.5, repeat: Infinity, ease: "easeInOut" }}
            className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[#0F172A]/80 backdrop-blur-md border border-white/20 shadow-md mb-2"
          >
            ...
            <span className="font-mono text-[9px] sm:text-[10px] font-bold uppercase tracking-[0.25em] text-[#F9F7F2]">
              Stratospheric Cruising Specification
            </span>
          </motion.div>
          ...
          <div className="pt-2">
            ...
            >
              ...
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="text-[#0F172A]">
      ...
      <div className="relative z-20 max-w-7xl mx-auto px-5 sm:px-8 md:px-14 w-full">
        <div className="grid grid-cols-3 gap-3 sm:gap-6 py-4 sm:py-5 border border-white/15 bg-[#0F172A]/80 backdrop-blur-xl rounded-2xl px-4 sm:px-6 shadow-md text-white">
          ...
          <div className="space-y-0.5 border-l border-white/15 pl-3 sm:pl-6">
            <span className="font-mono text-[8px] sm:text-[9px] uppercase tracking-wider text-white/60 block">
              Cabin Acoustics
            </span>
            <p className="font-mono text-xs sm:text-sm md:text-base font-bold text-white">
