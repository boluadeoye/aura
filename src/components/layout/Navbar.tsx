  ...
  return (
    <>
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        scrolled
          ? "bg-[#F9F7F2]/95 backdrop-blur-xl border-b border-[#0F172A]/8 py-3.5 shadow-sm"
        ...
        <div className="max-w-7xl mx-auto px-5 sm:px-8 md:px-14 flex items-center justify-between">
          <a href="#" className="flex items-center gap-3 group">
            <div className={`w-5 h-5 rounded-sm flex items-center justify-center transition-transform duration-500 group-hover:rotate-45 ${
            ...
            <nav className="hidden md:flex items-center gap-7">
              {navLinks.map((link) => (
                <a
                  key={link.label}
                  href={link.href}
                  className={`font-sans text-[11px] font-semibold uppercase tracking-[0.2em] relative py-1 ${
                    scrolled ? "text-[#0F172A]/70 hover:text-[#0F172A]" : "text-white/80 hover:text-white"
                  }`}
                >
                  {link.label}
                </a>
            ...
            <button
              ...
              className={`px-4 sm:px-5 py-2 rounded-full font-sans text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.18em] active:scale-95 transition-all flex items-center gap-2 cursor-pointer shadow-sm border ${
                scrolled
                  ...
                  : "bg-[#DCEE2D] text-[#0F172A] border-[#DCEE2D] hover:bg-white hover:text-[#0F172A]"
              }`}
            >
      ...
      <AnimatePresence>
        {mobileMenu && (
          <motion.div
            ...
            animate="opened"
            exit="closed"
            className="fixed inset-0 z-[100] bg-[#F9F7F2] flex flex-col justify-between p-6 sm:p-10"
          >
            <div className="flex items-center justify-between border-b border-[#0F172A]/10 pb-5">
              ...
              <button
                ...
                className="w-10 h-10 rounded-full bg-[#0F172A]/5 flex items-center justify-center text-[#0F172A]"
              >
                <XIcon />
              </button>
            </div>
            <div className="flex flex-col space-y-4 my-auto">
              {navLinks.map((link, idx) => (
                <a
                  ...
                  href={link.href}
                  onClick={() => setMobileMenu(false)}
                  className="font-serif text-3xl sm:text-4xl text-[#0F172A] flex items-baseline justify-between border-b border-[#0F172A]/5 pb-3"
                >
            ...
            <div className="space-y-4 pt-4 border-t border-[#0F172A]/10">
              <button
                onClick={() => { setMobileMenu(false); onOpenInquiry(); }}
                className="w-full py-4 rounded-xl bg-[#0F172A] text-white font-sans text-xs font-bold uppercase tracking-[0.2em]"
              >
                Begin Bespoke Commission
              </button>
